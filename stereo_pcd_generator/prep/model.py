...
class CameraModel():
    ...
    def generate_K_from_cm(self):
        """
        Generate K matrix from cm1 and cm2
        """
        camera_matrix = (self.cm1 + self.cm2) / 2
        baseline_mm = np.linalg.norm(self.T)

        K_txt = f"""{" ".join(map(str, camera_matrix.flatten()))}
{baseline_mm * 1e-3}
"""
        return K_txt
    ...
    def load_model(cls, cam_path) -> None:
        ...
        logger.info(f"Loading camera model at {cam_path}")
        ...
        logger.info(f"Camera model loaded, image_size: {camera.image_size}")
        return camera
...
if __name__ == "__main__":
    model = CameraModel.load_model(r"C:\Users\Andy\DCIM\camera_model.json")
    print(model.generate_K_from_cm())
